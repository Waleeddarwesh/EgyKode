...
- **`Action`** — what may be done, as `service:Operation`. `s3:*` grants every S3 action there is, which is the
...
> starts from deny and adds back only what actually fails.
...
> builds the identity the cluster and pipeline really use.
...
When Bob tries to log into AWS, AWS redirects him to Okta. Okta checks if Bob is still employed. If yes, Okta passes a SAML assertion to AWS, and AWS issues Bob temporary credentials based on his Okta group.
