...
> applies these controls to a running workload.
...
To fight this, a US Executive Order required software sold to federal agencies to
ship a **Software Bill of Materials (SBOM)**, and the practice spread from there.
An SBOM is a list of every dependency, sub-dependency and OS library inside your
image.

In this pipeline, Trivy generates an SBOM in CycloneDX format and **Cosign** signs
the image. An admission controller such as Kyverno or OPA Gatekeeper then verifies
that signature before the image is allowed to run. If there is no valid signature
from an identity you trust, the Pod is rejected — but only because you configured
the admission controller to check. Signing on its own rejects nothing.
...
> builds the non-root image all of this assumes.
