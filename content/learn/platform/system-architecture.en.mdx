...
A: A Public Subnet's route table has a route to an Internet Gateway, so an instance there with a public IP can be reached from the internet. A Private Subnet has no such route, so nothing on the internet can open a connection to it directly.
