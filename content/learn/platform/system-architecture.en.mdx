...
2. **The numbered column on the left.** That is the build pipeline, stages 1
   ...
   then update and push the manifests. You will build it in Chapter 23.
...
4. **The loop on the right**, from pipeline stage 9 to the Git repository to
   **Argo CD**, which syncs it back into the cluster. Nothing deploys by being
   ...
   GitOps loop, and it is the most important idea here — Chapter 27 is
   entirely about why the arrow points that way.
