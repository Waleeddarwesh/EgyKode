...
2. **Zero Trust Networking:** Inside the cluster we do not assume the network is safe. Six NetworkPolicies enforce a default-deny stance. The frontend is explicitly allowed to reach the two backends, and the backends the database — but a compromised frontend pod cannot scan the namespace, because sharing a namespace grants it nothing.
