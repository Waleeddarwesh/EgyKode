...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/git-branching-collaboration"
  local:
    enabled: true
    environment: base
    tools:
...
successCriteria:
  ...
  - text: "You can explain why removing a committed secret from history does not make it safe."
    verify: reasoning
...
<LabStep n={1} title="The daily loop" proves="You can take a change from branch to merged pull request with a history that stays linear" criterion={1}>
...
<LabStep n={2} title="Resolving a conflict properly" proves="You can resolve a conflict by reading both sides, rather than taking one wholesale" criterion={2}>
...
<LabStep n={3} title="The recovery everybody needs eventually" proves="You can recover a commit after a hard reset, using the reflog" criterion={3}>
...
<LabStep n={4} title="A secret in history" proves="You can remove a secret from history, and explain why that does not make it safe" criterion={4}>
