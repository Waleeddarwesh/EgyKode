...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/docker-networks-volumes-healthchecks"
  local:
    enabled: true
    environment: base
    tools:
...
<LabStep n={1} title="Names, not addresses" proves="You can reach a service by name, with no IP address written down anywhere" criterion={1}>
...
<LabStep n={2} title="Prove isolation is real" proves="You can explain why two containers on separate networks cannot see each other, having proved it" criterion={4}>
...
What you just observed is a **network namespace** — the kernel giving each
container its own interfaces, addresses and ports. Look at it directly:

```bash
docker exec lonely ip addr        # its own interfaces, not the host's
docker exec lonely hostname       # a UTS namespace: its own hostname
docker exec lonely ps aux         # a PID namespace: it is PID 1 to itself
```

Three namespaces, three commands. The isolation is not Docker's — Docker asked
the kernel for it, which is why the same primitives reappear under Kubernetes.

</LabStep>

<LabStep n={3} title="Volumes versus bind mounts" proves="You can choose between a volume and a bind mount by what survives a rebuild" criterion={2}>
...
<LabStep n={4} title="Started is not ready" proves="You can make a dependent service wait for readiness rather than for a process to exist" criterion={3}>
...
<LabStep n={5} title="Break each one deliberately" proves="You can break each mechanism on purpose and recognise the symptom it produces">
