...
title: "Kubernetes Security Hardening (NetworkPolicies) & HPA — Challenge"
...
handsOn:
  ...
  local:
    ...
    environment: k8s
    note: >-
      Run `./egykode cluster calico` first — on the default kind CNI a NetworkPolicy is
      accepted and ignored, so the challenge would pass without blocking any
      traffic.
    tools:
      - docker
      - kubectl
