...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/docker-multi-stage-build"
  local:
    enabled: true
    environment: base
    tools:
...
<LabStep n={1} title="The Dockerfile" proves="You can write a multi-stage build whose final image carries no compiler or build toolchain" criterion={1}>
...
<LabStep n={2} title="Layer order is the build-time win" proves="You can order layers so that changing application code does not reinstall dependencies" criterion={3}>
...
<LabStep n={3} title="An entrypoint that waits instead of crashing" proves="You can make a container wait for its database instead of crash-looping against it" criterion={4}>
...
<LabStep n={4} title="Build and measure" proves="You can measure the image you built and prove it runs as a non-root user" criterion={2}>
