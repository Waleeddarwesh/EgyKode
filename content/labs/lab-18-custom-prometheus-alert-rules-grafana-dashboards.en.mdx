...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/prometheus-alerts-dashboards"
  local:
    enabled: true
    environment: k8s
    tools:
...
successCriteria:
  - text: "An alert you wrote moves from inactive to pending to firing, and you caused it."
    verify: state
  - "The alert carries a runbook link and enough context to act on without opening a dashboard."
  - text: "A dashboard shows request rate, error rate and latency for your service."
    verify: state
  - text: "You can explain what `for:` does and why removing it produces pager noise."
    verify: reasoning
...
<LabStep n={1} title="A PrometheusRule" proves="You can write an alert, cause it to fire, and explain what for does to pager noise" criterion={[1, 4]}>
...
<LabStep n={2} title="Route it" proves="You can route an alert so it arrives carrying enough context to act on without opening a dashboard" criterion={2}>
...
<LabStep n={3} title="A dashboard, kept in Git" proves="You can build a dashboard of request rate, error rate and latency, and keep it in Git" criterion={3}>
