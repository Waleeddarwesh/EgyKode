...
<LabStep n={1} title="A database with backups on" proves="You can create a database with backups already on, rather than adding them after the first incident">
...
<LabStep n={2} title="Data worth losing" proves="You can put data in that you are willing to lose, so the recovery is a real one" criterion={2}>
...
<LabStep n={3} title="A manual snapshot" proves="You can take a manual snapshot, and say why it outlives the instance when automated backups do not" criterion={1}>
...
<LabStep n={4} title="The incident" proves="You can start the clock at the moment of the decision, which is where RTO is actually measured" criterion={2}>
...
<LabStep n={5} title="Restore" proves="You can restore from a snapshot and verify the data is back by querying it" criterion={2}>
...
<LabStep n={6} title="What you just measured" proves="You can state the RTO you measured, and explain why the restored instance answers on a different endpoint" criterion={[3, 4]}>
