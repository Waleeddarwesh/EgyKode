...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/k8s-storage-persistence"
  local:
    enabled: true
    environment: k8s
    tools:
...
successCriteria:
  ...
  - text: "The same data survives a Pod deletion once a PVC is mounted."
    verify: state
  - text: "You can explain why a 3-replica Deployment with one ReadWriteOnce PVC leaves Pods Pending."
    verify: reasoning
...
<LabStep n={1} title="Prove the filesystem is disposable" proves="You can demonstrate that a container filesystem is disposable rather than assume it" criterion={1}>
...
<LabStep n={2} title="The three objects" proves="You can name the three storage objects and what each one is responsible for">
...
<LabStep n={3} title="A claim, and a Pod that uses it" proves="You can mount a claim and show the same data surviving a Pod deletion" criterion={2}>
...
<LabStep n={4} title="The access mode that blocks a rollout" proves="You can explain why a three-replica Deployment on one ReadWriteOnce claim leaves Pods Pending" criterion={3}>
...
<LabStep n={5} title="What happens to the disk" proves="You can state what happens to the underlying disk when a claim is deleted, for your StorageClass" criterion={4}>
