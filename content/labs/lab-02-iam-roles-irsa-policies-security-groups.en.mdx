...
<LabStep n={1} title="A role is two policies" proves="You can write a role whose trust policy names one service and whose permissions name exactly what it needs" criterion={1}>
...
<LabStep n={2} title="IRSA — a role per workload, not per node" proves="You can let a Pod assume an IAM role with no credential file existing anywhere" criterion={4}>
...
<LabStep n={3} title="Security group chaining" proves="You can chain security groups to each other instead of hardcoding CIDR ranges, and prove the database refuses everyone else" criterion={[2, 3]}>
