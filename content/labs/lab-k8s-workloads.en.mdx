...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/k8s-workloads"
  local:
    enabled: true
    environment: k8s
    tools:
...
relatedPractice:
  - title: "Kubernetes scenarios"
    url: "https://killercoda.com/kubernetes"
  - title: "Kubernetes 101"
    url: "https://killercoda.com/k8s-101"
    note: "a set of beginner exercises"
successCriteria:
  - "A Deployment with 3 replicas is running, and you can name the ReplicaSet that owns the Pods."
  - text: "Deleting a Pod causes a replacement to appear; deleting the Deployment does not."
    verify: state
  - text: "A rolling update keeps the service available, and `rollout undo` returns to the previous image."
    verify: state
...
<LabStep n={1} title="A cluster on your laptop" proves="You can create a cluster on your own machine to work against">
...
<LabStep n={2} title="A bare Pod, and why you will not use one" proves="You can explain why a bare Pod is not something you deploy">
...
<LabStep n={3} title="A Deployment" proves="You can run a Deployment and name the ReplicaSet that owns its Pods, and show which deletions are repaired" criterion={[1, 2]}>
...
<LabStep n={4} title="A rolling update" proves="You can roll out a change without losing availability, and undo it" criterion={3}>
...
<LabStep n={5} title="Break it deliberately" proves="You can produce a Deployment that creates zero Pods, and explain why" criterion={4}>
