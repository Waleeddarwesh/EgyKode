...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/loki-log-queries"
  local:
    enabled: true
    environment: k8s
    tools:
    ...
    capabilities:
      - kubernetes-cluster
relatedPractice:
  - title: "Grafana Labs scenarios"
    url: "https://killercoda.com/grafana-labs"
    note: "Loki, Grafana and Tempo"
successCriteria:
...
<LabStep n={1} title="Install the stack" proves="You can install a logging stack and query logs from every namespace in one place" criterion={1}>
...
<LabStep n={2} title="Why label-only indexing" proves="You can explain why selecting by label first is what makes the query affordable, and what happens to a deleted Pod's logs with and without Loki" criterion={4}>
...
<LabStep n={3} title="Queries you will actually use" proves="You can filter to one Deployment's errors over a time window" criterion={2}>
...
<LabStep n={4} title="Cause a crash and find it" proves="You can cause a crash and find its cause from the logs alone, including the container that already died" criterion={3}>
...
<LabStep n={5} title="Retention, before it costs you" proves="You can bound how long logs are kept before the bill decides it for you">
...
<LabStep n={6} title="Metrics and logs together" proves="You can say which question belongs to metrics and which to logs">
