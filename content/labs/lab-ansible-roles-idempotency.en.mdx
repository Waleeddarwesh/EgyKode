...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/ansible-roles-idempotency"
  local:
    enabled: true
    environment: base
    tools:
...
relatedPractice:
  - title: "Ansible Handson"
    url: "https://killercoda.com/ansible"
    note: "writing and developing playbooks"
successCriteria:
...
<LabStep n={1} title="The role layout" proves="You can lay out a role so Ansible finds each part by name rather than by configuration">
...
<LabStep n={2} title="Tasks that tell the truth" proves="You can write tasks that take their values from variables, with nothing hardcoded" criterion={1}>
...
<LabStep n={3} title="Handlers: once, at the end" proves="You can restart a service exactly once at the end, through a handler rather than inline" criterion={3}>
...
<LabStep n={4} title="Prove it" proves="You can demonstrate changed=0 on a second run, rather than assuming it" criterion={2}>
...
<LabStep n={5} title="The two ways people break it" proves="You can name two ways a task silently breaks idempotency, and guard against them" criterion={4}>
