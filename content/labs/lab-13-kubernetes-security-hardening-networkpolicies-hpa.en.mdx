...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/k8s-networkpolicy-hpa"
  local:
    enabled: true
    environment: k8s
    note: >-
      Create the cluster with `./egykode cluster calico` for this one. kind's default
      CNI accepts NetworkPolicy objects and enforces none of them, so your policies
      would appear to apply while blocking nothing. HPA also needs metrics-server,
      which the same command installs.
    tools:
      - docker
      - kubectl
      - kind
...
relatedPractice:
  - title: "Kyverno scenarios"
    url: "https://killercoda.com/kyverno"
    note: "policy enforcement beyond NetworkPolicy"
  - title: "Falco scenarios"
    url: "https://killercoda.com/falco"
    note: "runtime threat detection"
successCriteria:
...
<LabStep n={1} title="Default deny — start by breaking it" proves="You can apply a default-deny policy and demonstrate that traffic is genuinely blocked, not merely accepted" criterion={[1, 4]}>
...
<LabStep n={2} title="Allow exactly what is needed" proves="You can allow exactly the traffic an application needs, including the DNS everyone forgets" criterion={2}>
...
<LabStep n={3} title="The HPA" proves="You can scale a workload on CPU, and explain why an HPA with no requests does nothing" criterion={3}>
