...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/jenkins-fundamentals"
  local:
    enabled: true
    environment: cicd
    tools:
...
<LabStep n={1} title="Jenkins that survives its container" proves="You can run Jenkins so its home survives the container being replaced" criterion={1}>
...
<LabStep n={2} title="A job that builds on push" proves="You can make a commit trigger a build without anyone pressing a button" criterion={2}>
...
<LabStep n={3} title="Role-based access" proves="You can create an administrator and a read-only user, and demonstrate that the second cannot start a build" criterion={[3, 4]}>
...
<LabStep n={4} title="Why this comes before pipelines" proves="You can say why persistence and access control come before pipelines, not after">
