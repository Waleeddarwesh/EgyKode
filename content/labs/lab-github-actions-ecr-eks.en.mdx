...
<LabStep n={1} title="A role GitHub can assume, with no stored key" proves="You can let a workflow assume an AWS role with no access key stored anywhere" criterion={2}>
...
<LabStep n={2} title="The workflow" proves="You can build an image tagged with the commit, scan it, and fail before pushing when the scan finds something serious" criterion={[1, 3]}>
...
<LabStep n={3} title="EKS has its own permission layer" proves="You can grant a role access to the Kubernetes API, and make the workflow fail when a rollout does not complete" criterion={4}>
...
<LabStep n={4} title="Jenkins or Actions?" proves="You can choose between Jenkins and Actions by what each one asks you to maintain">
