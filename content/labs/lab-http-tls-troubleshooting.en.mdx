...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/tls-certificate-diagnosis"
  local:
    enabled: true
    environment: base
    tools:
...
successCriteria:
  ...
  - text: "You can explain what `SSL_ERROR_SYSCALL` and `certificate verify failed` each imply."
    verify: reasoning
  - text: "You can prove a port is reachable independently of whether TLS succeeds."
    verify: state
...
<LabStep n={1} title="DNS" proves="You can settle whether a name resolves before blaming anything above it">
...
<LabStep n={2} title="TCP, without TLS in the way" proves="You can prove a port is reachable independently of whether TLS succeeds" criterion={4}>
...
<LabStep n={3} title="TLS" proves="You can read a certificate's subject, issuer and expiry from the command line, and say what a verification failure implies" criterion={[1, 3]}>
...
<LabStep n={4} title="HTTP" proves="You can separate an application error from a transport error in the response itself">
...
<LabStep n={5} title="Practise on deliberately broken endpoints" proves="You can name which layer failed for a broken URL you have not seen before" criterion={2}>
