...
handsOn:
  online:
    platform: killercoda
    enabled: true
    url: "https://killercoda.com/waleeddarwesh/scenario/git-branch-protection"
  local:
    enabled: true
    environment: base
    tools:
...
<LabStep n={1} title="A check for the branch to depend on" proves="You can create a check that has run at least once, which is what branch protection can then require" criterion={2}>
...
<LabStep n={2} title="Protect the branch" proves="You can protect a branch so direct pushes are rejected and a failing check blocks the merge, and say why requiring branches to be up to date matters" criterion={[1, 2, 4]}>
...
<LabStep n={3} title="CODEOWNERS" proves="You can route review automatically to the person responsible for a path" criterion={3}>
...
<LabStep n={4} title="Commit messages a machine can read" proves="You can write commit messages a release process can read">
...
<LabStep n={5} title="The pull request" proves="You can open a pull request that says what a reader can now do, not merely what changed" criterion={[2, 3]}>
